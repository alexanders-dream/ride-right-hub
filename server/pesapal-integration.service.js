...
export class PesaPalIntegrationService {
  constructor() {
    ...
    this.callbackBaseUrl = process.env.CALLBACK_BASE_URL || process.env.BACKEND_URL;
    ...
    // Initialize IPN registration for STK push
    this.initializeIPN();
  }
  ...
  async initializeIPN() {
    try {
      if (!this.callbackBaseUrl) {
        console.warn('⚠️ CALLBACK_BASE_URL not set, IPN registration skipped');
        return;
      }
      
      const ipnUrl = `${this.callbackBaseUrl}/api/payments/pesapal/ipn`;
   ...
   */
  async submitOrder(payment, customerDetails, billingAddress) {
    try {
      ...
      const orderRequest = {
        ...
        notification_id: this.ipnId || payment.id, // Use IPN ID if available, fallback to payment ID
        billing_address: {
          email_address: customerDetails.email,
          phone_number: customerDetails.phone,
          country_code: 'KE', // Required for M-Pesa STK push
   ...
   */
  validatePaymentData(payment, customerDetails, billingAddress) {
    const errors = [];
   ...
   */
  mapPesaPalStatusToInternal(pesaPalStatus) {
    const statusMap = {
      'COMPLETED': 'completed',
      'PENDING': 'pending',
      'FAILED': 'failed',
